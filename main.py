...
def _detect_columns(df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    ...
    """
    cat_cols = [
        c for c in df.columns if str(df[c].dtype) in ("object", "category", "bool")
    ]
...
def train_and_predict(cfg: Config) -> pd.DataFrame:
    ...
    if not fast_mode:
        skf = StratifiedKFold(
            n_splits=n_splits, shuffle=True, random_state=cfg.random_state
        )
    ...
    if fast_mode:
        ...
        print(
            f"Holdout AUC: {fold_auc:.5f}  |  Best iter: {getattr(model, 'best_iteration_', None)}"
        )
        ...
        for fold, (tr_idx, va_idx) in enumerate(skf.split(X, y), start=1):
            ...
            fold_auc = roc_auc_score(y_va, va_pred)
            print(
                f"Fold {fold}/{n_splits} AUC: {fold_auc:.5f}  |  Best iter: {getattr(model, 'best_iteration_', None)}"
            )
...
def main() -> None:
    ...
    print(
        f"Wrote submission to: {os.path.abspath(out_path)} | shape={submission.shape}"
    )
