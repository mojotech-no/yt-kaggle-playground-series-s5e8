...
def preprocess_data(train_df, test_df):
    ...
    X_train["age_group"] = pd.cut(
        X_train["age"], bins=[0, 25, 35, 50, 100], labels=[0, 1, 2, 3]
    ).astype(int)
    X_test["age_group"] = pd.cut(
        X_test["age"], bins=[0, 25, 35, 50, 100], labels=[0, 1, 2, 3]
    ).astype(int)
    ...
    # Duration categories
    X_train["duration_long"] = (
        X_train["duration"] > X_train["duration"].median()
    ).astype(int)
    X_test["duration_long"] = (
        X_test["duration"] > X_train["duration"].median()
    ).astype(int)
...
def create_ensemble(models, X_train, y_train, X_test, test_ids):
    ...
    weighted_pred = sum(
        weights[name] * test_predictions[name] for name in weights.keys()
    )
...
def main():
    ...
    # Preprocess data
    X_train, X_test, y_train, test_ids, label_encoders = preprocess_data(
        train_df, test_df
    )
    ...
    print(
        f"Best individual model: {best_model_name} (AUC: {scores[best_model_name]:.4f})"
    )
