...
class BankMarketingPredictor:
    ...
    def load_data(self):
        ...
        if not train_path.exists() or not test_path.exists():
            ...
            logger.info("Please ensure train.csv and test.csv are in the data directory")
            return False
        ...
        if len(self.train_df) > 100000:
            ...
            self.train_df = self.train_df.sample(n=100000, random_state=42).reset_index(drop=True)
    ...
    def explore_data(self):
        ...
        print("\nNumerical columns:")
        num_cols = self.train_df.select_dtypes(include=['int64', 'float64']).columns.tolist()
        if 'id' in num_cols:
            num_cols.remove('id')
        if 'y' in num_cols:
            num_cols.remove('y')
    ...
    def preprocess_data(self):
        ...
        for col in all_features.columns:
            if all_features[col].dtype == 'object':
                ...
                mode_val = all_features[col].mode()[0] if len(all_features[col].mode()) > 0 else 'unknown'
    ...
    def engineer_features(self, df):
        ...
        if age_cols:
            age_col = age_cols[0]
            df[f'{age_col}_group'] = pd.cut(df[age_col], 
    ...
    def train_lightgbm(self, X_train, y_train, X_val=None, y_val=None):
        ...
        lgb_model = lgb.LGBMClassifier(**lgb_params, n_estimators=100)  # Reduced from 300
    ...
    def make_predictions(self):
        ...
        for name, model in self.models.items():
            ...
            logger.info(f"{name.upper()} predictions range: [{pred.min():.4f}, {pred.max():.4f}]")
        ...
        logger.info(f"Ensemble predictions range: [{ensemble_pred.min():.4f}, {ensemble_pred.max():.4f}]")
    ...
    def create_submission(self, predictions):
        ...
        if sample_sub_path.exists():
            ...
            submission = pd.DataFrame({
                'id': self.test_df['id'] if 'id' in self.test_df.columns else range(len(predictions)),
                'y': predictions
            })
